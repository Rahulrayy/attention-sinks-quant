...
> **Status: inference-side audit complete.** Five checkpoints measured
> end-to-end across 8 / 6 / 4-bit activations — **360 grid cells** on 281
> FineWeb-Edu documents, a **60-cell replication at all three widths** on a
> second corpus of Python source, and one downstream task (LAMBADA, 1000
> examples — the only metric here that is not perplexity). All four
> pre-registered fp16 exception arms have been executed; two of them turned out
> never to have had a working code path, which §5.9 reports rather than hides.
>
...
> deliberately — finding first, architecture last. A pretraining arm
> (`train/`, `softmax1`) is specified and stubbed but was descoped; §6 says what
> that costs.
...
made afterwards is recorded in §6 rather than applied quietly.
...
the sorted ratios. §5.3's finding does not move with it; one sentence in §5.5
does, and says so.
...
(§6) and had missed at 8. The claim survives the
...
This was predicted. That cell was on record as the one borderline
...
- `D_sink` is **undefined** for the gated arms in the `detected_sinks` exception
  ...
  missing data — it is what sink-free means (§6).
...
sensitivity has always been that fact (§6): the one swap already
...
outlier supplying most of the apparent correlation, which was flagged as a risk
at the time; this is that risk arriving.
...
not measurements. §6 has the rest of what this task does not license.
...
The open question this arm was run to answer had two branches: if it rescued `1B_elementwise` and only that, C23's axis story was
...
detector does not watch. §6 has what the arm does not license, including the
fact that it is one channel and not a set.
...
multi-level sink literature that motivated the detector (§2). See §6.
...
10 of 10 — the instability is the corpus slice, not the code. See §6.

## 6. Limitations

Twenty-four were recorded over the project, the first six before any code ran.
These are the ones that constrain the results above; they are listed here rather
than in a separate file because a caveat kept somewhere else is a caveat nobody
reads.
...
- **The controlled triple is one lab's training run.** Baseline, head-wise and
...
- **The element-wise arm is not parameter-matched** (+12%, because the gate is
  fused into `q_proj`). §5.3's finding on that arm confounds gating with
  capacity. The *ranking* inverts either way; the cause is not isolated. The
  head-wise arm at +0.1% is the matched pair, and is where the headline claim
  is measured.
- **Three corpora, all English**, all 1.2M characters, read whole-file at
  seq_len 256. The domain swap bounds the corpus sensitivity for the first time
  and the bound is uneven — interventions with controls travelled, a rank
  correlation did not (§5.7). Nothing here speaks to other languages or long
  context.
- **The held-out slice is 32 sequences.** Every confidence interval in this
  README is a bootstrap over those.
- **The pretraining arm was descoped, and it is the one gap §3 opened and did
  not close.** `softmax1` is widely cited and has never been evaluated with
  seeds; testing it needs pretraining from scratch, because the identity
  `softmax1(x) = s · softmax(x)` means a retrofit degrades a model for reasons
  that say nothing about the mechanism (§4). The scaling identity is implemented
  and tested (`tests/test_softmax1.py`) and the trainer is a contract-only stub.
  So this repo says nothing about softmax1, and the table in §3 marks it as
  still unevaluated rather than quietly dropping the row.

**On the method.**

- **Fake quantization, not deployed quantization.** Weights and activations are
  quantized and dequantized in fp16; there is no int8 kernel and no speedup
  measured. The arithmetic error is real, the systems claim is not made.
...
- **The metric was refined before data collection, and the refinement is on
  ...
  τ=10. Both changes predate the results.
- **The calibration draw is not a variance source for the per-token arm.** Five
  ...
  instead. A project auditing single-run numbers found its own variance plan
  half-void, which is recorded rather than quietly patched.

**On what the numbers mean.**

- **The destroyed-cell threshold is a judgement call** (§4). Cells above 10×
  their own reference are flagged and excluded from ratio columns.
  `--threshold-sweep` prints its sensitivity. §5.3 does not move with it; one
  sentence in §5.5 does.
- **The 4-bit grid is uninterpretable** in both granularities, and is reported
  as such rather than dropped or spun. Ranking two models that are both
  destroyed ranks nothing.
- **`D_sink` is undefined for a sink-free model.** With no sink positions there
  ...
  means, and the same applies on the feature axis to a model with no outlier
  channels (§5.9).
- **`detected_sinks` adds no degree of freedom.** On every model where it is
  ...
  `position_0` arm. The grid has three real exception arms, not four.
- **The feature-axis arm is one channel, not a set**, and reaches the 140 of 196
  modules that share the residual stream's width — `o_proj` and `down_proj` read
  different feature axes, where a residual channel index means nothing.
- **LAMBADA is one task, in one language, at one sample size.** 1000 of 5153
  examples. `--power` prints each cell's resolution: a drop smaller than that is
  a bound, not a measurement, and several per-token cells are exactly that.
  Accuracy also saturates — once a model is at chance, further damage cannot
  register.
...
- **No Qwen checkpoint in this roster has a BOS token**, so four of five models
  ...
  within-model, and on all five draws prepending BOS moves its sink mass ~0.6%
  and changes nothing the detector selects.
...
Prior work is credited per claim in §3. The three papers the audit is built
against are Xiao et al. (StreamingLLM), Sun et al. (massive activations), and
Qiu et al. (gated attention, NeurIPS 2025) — whose released checkpoints are the
controlled triple every result here rests on.
