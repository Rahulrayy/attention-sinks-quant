...
**Date:** 26 August 2026
**State:** Track A complete across three bit widths, has an answer, and the one
open mechanism is now closed. Track B not started.
...
| Diagnostic runs | **14** — arm decomposition + eight localisation tests |
| Distribution runs | **5** — per-layer activation shape, one per checkpoint |
| Sink measurement runs | **5** — one per checkpoint, draw 0, no-BOS |
| Corrections to the original plan | **20** |
...
- **C20** — the previous handoff's "the damage is distributed across the
  network" was an inference from two failed searches, not a measurement. It is
  retracted, and the write-up rule that follows it is in §10.
...
- **`analysis/distributions.py`** (308 lines) — the R6 tables, joined against
  the damage they must explain. `--sweep` reports the thresholds at which the
  ranking fails, and finds two.

### Done in the handoff before this one
...
- **A second corpus.** Now unambiguously the largest unbounded sensitivity
  (LIMITATIONS §18), and the top of §11.
- **R6 at 6 and 4 bits.** The dispersion statistic is bit-width-dependent by
  construction and has only been run at 8 (LIMITATIONS §21).
...
Five findings. R1, R3-rev, R4 and R5 were in the last handoff; **R6 is new, and
it closes the mechanism R4 had to leave open** — retracting, in the process, a
sentence the last handoff wrote about that mechanism (C20).
...
**What is weak, and is stated in the README too.** The cross-model ranking
(annihilated-layer count orders the roster) rests on five models and holds at
thresholds ≥90% while **failing at 50% and 70%** — run
`python -m analysis.distributions --sweep`. And *why* the gate reshapes that
tensor in training is not explained. LIMITATIONS §21.
...
Twenty corrections to the original plan, every one established by running code
...
**Done and closed at 8 bits** (§3, R6) — this replaces the previous handoff's
"chase the element-wise fragility, or state it and stop", which is answered.
What remains is cheap and bounded: `dispersion`, `eff_bits` and both underflow
fractions are computed against a specific integer grid, so every R6 number is an
*8-bit* number. `python -m quant.distributions --model <m> --bits 6` is one
command per checkpoint and would say whether the layer-0 tensor is the story at
6 bits too, where per-tensor destroys four of five models anyway (§3, R5).
